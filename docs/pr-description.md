...
- 선택한 검증 방법: hook 문법 확인, pre-commit hook 실행, Docker compose 기동, 실제 앱 화면 캡처를 통해 문서와 workflow가 동작 가능한 상태인지 확인했습니다.

## 가장 막혔던 순간

- 막힌 지점: Docker compose 환경에서 frontend가 browser bundle을 빌드할 때 `NEXT_PUBLIC_API_BASE_URL`이 runtime 환경변수만으로는 반영되지 않는 점이 헷갈렸습니다.
- AI에게 다시 질문한 방식: “Next.js public 환경변수는 Docker build와 runtime 중 어느 단계에서 반영되는가?”를 기준으로 원인을 좁혔습니다.
- 해결 방법: Docker build arg로 API base URL을 주입하고, Playwright base URL도 환경변수로 분리해 compose 환경에서 검증할 수 있게 했습니다.
